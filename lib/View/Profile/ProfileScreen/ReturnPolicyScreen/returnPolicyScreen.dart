...
class _ReturnPolicyScreenState extends State<ReturnPolicyScreen> {
  ...
  Future<void> fetchData() async {
    ...
    final returnPolicyViewmodel =
        Provider.of<ReturnPolicyViewmodel>(context, listen: false);
    returnPolicyViewmodel.fetchReturnPolicyApi(
      ...
      body: FutureBuilder<void>(
        ...
        builder: (context, snapshot) {
          if (snapshot.connectionState == ConnectionState.waiting) {
            ...
            return Consumer<ReturnPolicyViewmodel>(
              builder: (context, value, _) {
                switch (value.returnPolicy.status!) {
                  case Status.LOADING:
                    return Center(child: CircularProgressIndicator());
                  case Status.ERROR:
  ...
  AppBar _buildAppBar() {
    return AppBar(
      ...
      title: Text('Returns Policy', style: TextStyle(color: Colors.black)),
      actions: [
        IconButton(
  ...
  Widget handleError(ReturnPolicyViewmodel value) {
    String? message;
    if (value.returnPolicy.message != "No Internet Connection") {
      String jsonString = value.returnPolicy.message.toString();
    ...
    return value.returnPolicy.message == "No Internet Connection"
        ? ErrorScreenWidget(
            onRefresh: () async {},
            loadingText: value.returnPolicy.message.toString(),
          )
        : Text('data');
  }
  ...
  Widget _buildCompletedUI(
    ...
    ReturnPolicyViewmodel termsAndConditionsViewmodel,
  ) {
    List<dynamic> content = json.decode(termsAndConditionsViewmodel
        .returnPolicy.data!.returnPolicyContentList![0].policyContent
        .toString());
